...
void convert_long_to_hex_char(long* quat, unsigned char* hex, int numElement)
{
    ...
    for (int index = 0; index < numElement; index++) {
        for (int i = 0; i < 4; i++) {
            ...
            //ALOGI("e%d quat[%d]: %x", index, bytePosition, hex[bytePosition]);
            bytePosition++;
        }
    }
    return;
...
void dump_dmp_img(const char *outFile)
{
    ...
    ALOGI("HAL DEBUG:dump DMP image");
    ALOGI("HAL DEBUG:open %s\n", dmp_path);
    ALOGI("HAL DEBUG:write to %s", outFile);
...
int read_sysfs_dir(bool fileMode, char *sysfs_path)
{
    ...
    if (dp != NULL)
    {
        ALOGI("******************** System Sysfs Dump ***************************");
        ALOGV_IF(0,"HAL DEBUG: opened directory %s", sysfs_path);
        while ((ep = readdir (dp))) {
            if(ep != NULL) {
                ALOGV_IF(0,"file name %s", ep->d_name);
                ...
                ALOGV_IF(0,"HAL DEBUG: reading %s", full_path);
                ...
                if (fd > -1) {
                    ...
                    if (res > 0) {
                        ...
                        if (res)
                            ALOGI("HAL DEBUG:sysfs:cat %s = %ld", full_path, data);
                    } else {
                         ALOGV_IF(0,"HAL DEBUG: error reading %s", full_path);
                    }
                } else {
                    ALOGV_IF(0,"HAL DEBUG: error opening %s", full_path);
                }
                close(fd);
            }
        }
        ...
        ALOGI("HAL DEBUG: could not open directory %s", sysfs_path);
    }
