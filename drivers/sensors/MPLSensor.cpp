                         ...
                         mAccelOrientation{0}
{
    ...
    if (inv_get_chip_name(chip_ID) != INV_SUCCESS) {
        ...
        else if (strcmp(chip_ID, "MPU9150"))
            mIntegratedAccel = true;
        else
 ...
 *  Puts MPU in low power state except for accelerometer
 *  en: false = disable
 *      true = enable
 *  ms: accelerometer poll period in milliseconds
 *  MPU HW provides for four possible polling
 *  periods: 800, 400, 200, and 100ms
 */
int MPLSensor::lpa_enable(bool en, unsigned int ms)
{
    long attr;
    int fd;
    int err = 0;
    unsigned int hw[4] = {800, 400, 200, 100};

    if (mIntegratedAccel) {
        fd = open(mpu.lpa_freq, O_RDWR);
        for (attr = 0; attr < 3; attr++) {
            if (ms >= hw[attr])
                break;
        }
        err = write_attribute_sensor(fd, attr);
        if (err <= 0)
            return err;

        fd = open(mpu.lpa_mode, O_RDWR);
        if (en)
            attr = 1;
        else
            attr = 0;
...
int MPLSensor::writeAccelFifoRate(uint64_t ns)
{
    int fd;
    int err;
    unsigned int ms = (unsigned int)(ns / 1000000L);
    long attr = (long)(1000000000.f/ns);

    fd = open(mpu.accel_fifo_rate, O_RDWR);

    /* REMOVE NEXT LINE WHEN KERNEL DRIVER MIN_FIFO_RATE IS UPDATED */
    if (attr < 4) attr = 4;

    err = write_attribute_sensor(fd, attr);
    if (err <= 0) {
        ALOGE("%s ERR setting %s to %ums", __func__, mpu.accel_fifo_rate, ms);
    } else {
        if (ns < DEFAULT_POLL_PERIOD) {
            lpa_enable(false, 3);
            ALOGV_IF(SYSFS_VERBOSE,
                     "Changed %s to %ums", mpu.accel_fifo_rate, ms);
        } else {
            err = lpa_enable(true, ms);
            if (err > 0)
                ALOGV_IF(SYSFS_VERBOSE,
                         "Set accelerometer low power polling to %ums", ms);
            else
                ALOGE("ERR setting accelerometer low power polling to %ums",
                      ms);
        }
...
int MPLSensor::inv_init_sysfs_attributes(void)
{
    ...
    sprintf(mpu.lpa_mode, "%s%s", sysfs_path, "/lpa_mode");
    sprintf(mpu.lpa_freq, "%s%s", sysfs_path, "/lpa_freq");
