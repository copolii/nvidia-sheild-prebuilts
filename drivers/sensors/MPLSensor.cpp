                         ...
                         mAccelOrientation{0}
{
    ...
    mForceSleep = false;
    mAccelThreshold = ACCEL_THRESHOLD;
    /* setup sysfs paths */
    ...
    if (inv_get_chip_name(chip_ID) != INV_SUCCESS) {
        ...
        ALOGV("HAL:Chip ID=%s\n", chip_ID);
        if (!strncmp(chip_ID, "MPU9250", strlen("MPU9250")) ||
            !strncmp(chip_ID, "MPU6500", strlen("MPU6500")) ||
            !strncmp(chip_ID, "MPU6515", strlen("MPU6515"))) {
            mAccelThreshold = MPU6500_ACCEL_THRESHOLD;
        }
    }
...
bool MPLSensor::isDeviceOrientationStable()
{
    ...
    if (mPollPeriod < DEFAULT_POLL_PERIOD) {
        ...
        if (diff[0] > mAccelThreshold ||
            diff[1] > mAccelThreshold ||
            diff[2] > mAccelThreshold) {
            stable = false;
        }
    } else {
        if (false == mWindowFull) {
            ...
            diff[2] = fabs((ACCEL_WINDOW_SIZE*
                        ...
                        - mAccelWindowSum[2]);
            if (diff[0] > ACCEL_WINDOW_SIZE*mAccelThreshold ||
                diff[1] > ACCEL_WINDOW_SIZE*mAccelThreshold ||
                diff[2] > ACCEL_WINDOW_SIZE*mAccelThreshold) {
                stable = false;
            }
        }
    }
