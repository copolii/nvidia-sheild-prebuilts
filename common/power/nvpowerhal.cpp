...
void common_power_open(struct powerhal_info *pInfo)
{
    ...
    pInfo->features.smart_dimmer = sysfs_exists("/sys/devices/platform/host1x/tegradc.0/smartdimmer/enable");
...
static void set_smart_dimmer(struct powerhal_info *pInfo, bool on)
{
    ALOGV("%s: %s", __func__, PATH_SMARTDIMMER_ENABLE);

    if (!pInfo->features.smart_dimmer)
    {
        ALOGE("%s: smart_dimmer does not exist", __func__);
        return;
    }

    ALOGV("%s: %d", __func__, on);
    sysfs_write_int("/sys/devices/platform/host1x/tegradc.0/smartdimmer/enable", on);
}
...
static void set_camera_hint(struct powerhal_info *pInfo, camera_hint_t *data)
{
    ...
    if (data[0] != CAMERA_HINT_RESET)
    {
        set_smart_dimmer(pInfo, 0);
    }
    else
    {
        set_smart_dimmer(pInfo, 1);
    }
                            ...
                            power_hint_t hint, void *data)
{
    ...
    case POWER_HINT_APP_LAUNCH:
        // Boost to MAX frequency for 2 cores
        pInfo->mTimeoutPoker->requestPmQosTimed(PMQOS_CONSTRAINT_CPU_FREQ,
                                                PM_QOS_BOOST_PRIORITY,
                                                PM_QOS_DEFAULT_VALUE,
                                                INT_MAX,
