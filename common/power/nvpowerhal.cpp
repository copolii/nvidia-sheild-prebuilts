...
void common_power_open(struct powerhal_info *pInfo)
{
    ...
    for (i = 0; i < pInfo->num_available_frequencies; i++)
    {
        if (pInfo->available_frequencies[i] >= 828000) {
...
static void set_cpu_scaling_max_freq(struct powerhal_info *pInfo, int value)
{
    if (value > 0) {
        ...
        pInfo->fds.app_max_freq = pInfo->mTimeoutPoker->requestPmQos(PMQOS_MAX_CPU_FREQ, value);
        ALOGV("%s: set max CPU ceiling =%d", __func__, value);
    }
}

static void set_cpu_scaling_max_freq_percent(struct powerhal_info *pInfo, int percent)
{
    ...
    if (percent > 0 && percent <= 100) {
        if (pInfo->cpu0_max_frequency >= 0) {
            ...
            set_cpu_scaling_max_freq(pInfo, targetMaxFreq);
        }
    } else {
...
static void set_cpu_core_cap(struct powerhal_info *pInfo, int value)
{
    if (value < 0)
    ...
    pInfo->fds.app_max_online_cpus = pInfo->mTimeoutPoker->requestPmQos(PMQOS_MAX_ONLINE_CPUS, value);
    ...
    for (i = 0; i < APP_PROFILE_COUNT; i++)
    {
        switch (i) {
            ...
            case APP_PROFILE_CPU_MAX_NORMAL_FREQ_IN_PERCENTAGE:
                ...
                set_cpu_scaling_max_freq_percent(pInfo, data[i]);
                break;
            case APP_PROFILE_CPU_MAX_CORE:
                set_cpu_core_cap(pInfo, data[i]);
                break;
