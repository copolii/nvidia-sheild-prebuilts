...
if [ "$1" == "--help" ]; then
    ...
    echo "where:"
    echo " "
    echo "  target_board: exuma_sim (default is exuma_sim)"
...
if [ "$1" == "" ]; then
    echo "Assuming target board is exuma_sim"
    target_board="exuma_sim"
else
    target_board=$1
fi

firmware=""
if [ "$target_board" == "bonaire_sim" ]; then
    firmware="tegra12x"
fi
if [ "$target_board" == "exuma_sim" ]; then
    firmware="tegra13x"
fi
...
if [ "$firmware" == "tegra13x" ]; then
    mkdir -p $product_dir/nvtest_ramdisk_tmpdir/system/etc/firmware/tegra13x
    pushd $product_dir/nvtest_ramdisk_tmpdir/system/etc/firmware/tegra13x
    cp $product_dir/system/etc/firmware/tegra13x/* .
    popd
fi
