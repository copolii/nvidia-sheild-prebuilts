...
if [ $vendor = $TI ]; then
	...
	if [ "$boardid" == "0668" ] && [ "$fab" == "00" ]; then
		insmod /system/lib/modules/compat/wl18xx.ko dc2dc=0 low_band_component=0x2 low_band_component_type=0x5 high_band_component=0x1 high_band_component_type=0x9 n_antennas_5=0 n_antennas_2=1 forced_fw_ver=1168
	elif [ "$boardid" == "0668" ]; then
		insmod /system/lib/modules/compat/wl18xx.ko dc2dc=0 low_band_component=0x2 low_band_component_type=0x5 high_band_component=0x1 high_band_component_type=0x9 n_antennas_5=0 n_antennas_2=1
	else
